...
ROOT_SWEEPER_SCRIPT = SCRIPTS_DIR / "root_sweeper.py"
...
def main():
    ...
    # Step 0: Root Sweeper (최우선)
    logger.info("📍 Step 0/5: Root Sweeper - 루트 청소")
    logger.info("-" * 70)
    returncode, stdout, stderr = run_script(ROOT_SWEEPER_SCRIPT)
    print(stdout)
    if stderr:
        print(stderr, file=sys.stderr)

    if returncode != 0:
        logger.warning("⚠️  Root Sweeper 실행 중 오류 발생 (계속 진행)")
    logger.info("")

    # Step 1: Readwise Bridge
    logger.info("📍 Step 1/5: Readwise Bridge 실행")
    logger.info("-" * 70)
    ...
    logger.info("📍 Step 2/5: Google Docs 다운로드 시도")
    logger.info("-" * 70)
    ...
    # Step 4: Status Check
    logger.info("📍 Step 4/5: 시스템 상태 확인")
    logger.info("-" * 70)
