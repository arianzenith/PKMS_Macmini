...
1) 반드시 아래 순서/라벨을 지켜라: (인사) → 1. 거대 가설 → 2. 충돌 지점 → 3. 창발 아이디어 → 10. 파괴적 질문 → [이 질문이 치명적인 이유]
2) '📚 출처' 섹션은 절대 출력하지 마라. (출처는 시스템이 헤더에서 제공한다)
3) 3. 창발 아이디어에는 반드시 "오늘 바로 실행" 1개를 포함하라.
...
def build_prompt(rw_pick: list[dict], memo_pick: list[dict], oth_pick: list[dict]) -> str:
    ...
    return (
        "당신은 '생각공장' 아침 리포트 엔진이다.\n"
        "외부지성(Readwise)과 내부생각(메모), 그리고 기타(참고)를 충돌시켜"
        "의사결정에 유의미한 긴장/아이디어/질문을 생성하라.\n\n"
        f"{_RULES}\n\n"
        f"{_TEMPLATE}\n\n"
...
def ensure_sections(text: str) -> str:
    ...
    """
    needed = ["1. 거대 가설", "2. 충돌 지점", "3. 창발 아이디어", "10. 파괴적 질문", "[이 질문이 치명적인 이유]"]
    for k in needed:
        if k not in text:
            text += f"\n\n{k}\n(누락됨 — 다음 실행에서 재생성 필요)"
    return text
